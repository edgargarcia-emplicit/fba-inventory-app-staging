...
per-client Amazon FBA inventory dashboards fed by Google Sheets, hosted
for free.

## What's included

**Core dashboard**
- Multi-client dashboard with client selector, hourly-cached sync + Sync Now button
- KPI cards, sortable/searchable inventory table, DOI color flags
- Replenishment suggestions (units + cases), Order-By dates, Overdue/Urgent/Upcoming status
- Per-client Target DOI and Lead Time settings
- Mock shipment simulator, per-SKU notes, "mark as checked" tracking
...
- **DOI History** — daily DOI snapshots per SKU (one per day, taken automatically when the dashboard loads), with trend charts and tracking stats
- **Health Report** — stranded + unfulfillable inventory, synced from a Google Drive folder per client (same as the original; see setup below)
- **Notifications banner** on the dashboard — overdue reorders, aged-inventory fee exposure, stranded/unfulfillable counts, all in one place
- **Trending SKUs** and **Aged Inventory** panels (🔥 Hot / 📈 Rising flags, 91–365+ day aging buckets, AIS fee-unit exposure)
...
- **Shipments page** — shipment log with FBA shipment ID, tracking number (auto-detects UPS/FedEx/USPS/Amazon/DHL and links to the carrier's tracking page), and status; SKU case-pack and dimension management; a shipment-file builder that generates a downloadable `.xlsx`
